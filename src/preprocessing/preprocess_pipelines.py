...
========================
Full preprocessing pipelines cho IM2LATEX và CROHME.

Run standalone:
    python -m src.preprocessing.pipelines
"""
...
if 'name' == 'main':
    # Test the pipelines with sample images
    im2latex_sample = 'path/to/im2latex/sample.png'
    crohme_sample = 'path/to/crohme/sample.png'
    
    im2latex_processed = preprocess_im2latex(im2latex_sample, augment=True)
    crohme_processed = preprocess_crohme(crohme_sample, augment=True)
    
    print("IM2LATEX processed shape:", im2latex_processed.shape if im2latex_processed is not None else "Error")
    print("CROHME processed shape:", crohme_processed.shape if crohme_processed is not None else "Error")